...
FRAMES_PER_BUFFER = 8192  # how much audio data we read/write at a time
RECORD_SECONDS = 31       # how many seconds we of audio we keep in memory. determines max delay
MIN_DELAY = 0
...
class RadioDelay(object):
	...
	"""
	set up audio streams and audio data buffer array
	"""
	...
	"""
	setup for ssd mode (raspberry py)
	"""
	def init_ssd(self):
	...
	"""
	clear display, write line of text to display and show
	"""
	...
	"""
	thread callback for audio streams
	"""
	...
	"""
	iteration over our recorded chunks list
	add a new chunk from input to buffer array and output an offset chunk
	"""
	...
	"""
	handle keyboard input after enter pressed
	"""
	def input_loop(self):
